...
/**
 * PARSE ORDER
 * Uses Claude to convert natural language into a clean JSON array.
 */
async function parseOrder(message) {
  ...
  try {
    const response = await anthropic.messages.create({
      model: 'claude-3-5-haiku-latest', // Updated to the most stable current model
      max_tokens: 1000,
      messages: [{ 
        role: 'user', 
        content: `You are an ordering assistant. Convert this message into a JSON array of items based on the mapping provided.
        
        Mapping:
        ${itemMapStr}
        
        Rules:
        - Only include items from the map.
...
app.post('/whatsapp', async (req, res) => {
  res.status(200).send('EVENT_RECEIVED'); // Immediate response to Twilio
